...
template <typename T>
TH1 *createHistogram(const char *name, const char *title, int nBin, float binMin, float binMax);

//-----------------------------------------------------------------------------------------------------

template <>
TH1 *createHistogram<float>(const char *name, const char *title, int nBin, float binMin, float binMax)
{
...
template <>
TH1 *createHistogram<double>(const char *name, const char *title, int nBin, float binMin, float binMax)
{
...
template <>
TH1 *createHistogram<int>(const char *name, const char *title, int nBin, float binMin, float binMax)
{
...
template <>
TH1 *createHistogram<short>(const char *name, const char *title, int nBin, float binMin, float binMax)
{
...
TH1 *createHistogram(const char *name, const char *title,
		int nBin, float binMin, float binMax, const std::vector<T> &data)
{
...
template <typename T>
TH1 *createHistogram(const char *name, const char *title,
		int nBin, float binMin, float binMax, const Distribution<T> &distribution)
{
...
typedef std::map<DrawAttributeType, float> DrawAttributeMap;
...
{
	if(ARBOR_PFA == type) return "ArborPFA";
	else if(PANDORA_PFA) return "PandoraPFA";
	else return "UNKNOWN";
}
...
{
	if(SIMULATION == type) return "SIMULATION";
	else if(TEST_BEAM == type) return "TEST_BEAM";
	else if(FTFP_BERT_HP == type) return "FTFP_BERT_HP";
	else if(FTF_BIC == type) return "FTF_BIC";
	else return "UNKNOWN";
}
...
T Mean(const std::vector<T> &vec)
{
  ...
  return mean / static_cast<float>(vec.size());
...
T RMS(const std::vector<T> &vec)
{
	...
	return std::sqrt( meanSquare / vec.size() - mean*mean );
}
